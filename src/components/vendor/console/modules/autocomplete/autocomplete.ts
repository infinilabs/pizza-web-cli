...
import { Position, Range } from "../../entities/position";
import { Token } from "../../entities/Token";
import { CoreEditor } from "../../entities/core_editor";
import { AutoCompleteContext, ResultTerm } from "../../entities/autocomplete";
import RowParser from "../../entities/row_parser";

import * as utils from "../../utils/autocomplete";
// @ts-ignore
import { populateContext } from "./engine.js";
// @ts-ignore
import { URL_PATH_END_MARKER } from "./components/accept_endpoint_component.js";
import { CommandAutocompleteComponent } from "./components/command_autocomplete_component";

import {getFields } from "../../modules/mappings/mappings.js";
import { IndexAutocompleteComponent } from "./components/index_autocomplete_component";

function getCommandComponents() {
  return [new CommandAutocompleteComponent("command", null, true)];
}
...
export function getCurrentMethodAndTokenPaths(
  ...
  ret.tokensLineCache = tokenIter.tokensLineCache;
  ...
  if (tokenIter.getCurrentPosition().lineNumber === startPos.lineNumber) {
    if (
      ...
      (t.type === "url.part" ||
        t.type === "url.param" ||
        t.type === "url.value") || t.type === "url.select_keyword"
    ) {
      // we are forcing the end of the url for the purposes of determining an endpoint
      if (forceEndOfUrl && (t.type === "url.part" || t.type === "url.select_keyword")) {
  ...
  while (t && isUrlParamsToken(t)) {
    switch (t.type) {
      ...
      case "url.questionmark":
        ...
        ret.urlParamsTokenPath.unshift(
          (curUrlPart as Record<string, string>) || {}
        );
        curUrlPart = null;
        break;
    }
...
export default function({ coreEditor: editor }: { coreEditor: CoreEditor }) {
  ...
  function applyTerm(term: {
    ...
    if (
      ...
      if (term.template.__raw && term.template.value) {
        ...
        indentedTemplateLines = utils
          .jsonToString(term.template, true)
          .split("\n");
      }
  ...
  function addBodyPrefixSuffixToContext(context: AutoCompleteContext) {
    ...
    switch (nonEmptyToken ? nonEmptyToken.type : "NOTOKEN") {
      ...
      case "punctuation.colon":
        ...
        context.rangeToReplace!.end.column =
          (tokenIter.getCurrentTokenColumn() as number) +
          nonEmptyToken.value.length;
  ...
  function extractIndicesFromTokens(tokens: any[], currentTokenIndex: number){
    const indices = [];
    let iidx = -1;
    (tokens || []).forEach((token: any, idx: number)=>{
      const tv = token.value.toUpperCase();
      if(["FROM", "JOIN"].includes(tv)){
        if(tokens.length > idx + 2  && idx + 2 != currentTokenIndex){
          let index = tokens[idx+2].value;
          if(!index.includes("(")){
  ...
  function addPathAutoCompleteSetToContext(
    context: AutoCompleteContext,
    pos: Position
  ) {
    ...
    if (ret.method === "LOAD") {
    ...
    }else if(ret.method === "SELECT"){
      const curTokenIdx = (ret.tokensLineCache || []).findIndex(t=>t.value === ret.token.value);
      let {indices, idx} = extractIndicesFromTokens(ret.tokensLineCache, curTokenIdx);
      if(idx> -1 && curTokenIdx < idx) {
        const fields = getFields(indices);
      ...
      const length = ret.otherTokenValues?.length || 0;
      if(length > 0){
        switch(ret.otherTokenValues[length-1].toUpperCase()){
          case "FROM":
            if(!ret.token!.value.endsWith('"') && ret.token!.value != '"'){
              context.otherTokenValues = null;
              context.urlTokenPath = [];
              components = [new IndexAutocompleteComponent("indices", null, true)];
            }
            break;
          case "WHERE":
          case "AND":
          case "BY":
            if(length > 1 && !ret.token!.value.includes('=')){
              if(indices === ""){
                indices = ret.otherTokenValues[length-2];
            ...
            // components = [new FieldAutocompleteComponent("fields", null, false)]
        }
       
      }

    }else if(ret.method == "FROM"){
      const curTokenIdx = (ret.tokensLineCache || []).findIndex(t=>t.value === ret.token.value);
      if(ret.tokensLineCache[curTokenIdx - 2].value == "FROM"){
        if(!ret.token!.value.endsWith('"') && ret.token!.value != '"'){
          context.otherTokenValues = null;
          context.urlTokenPath = [];
          components = [new IndexAutocompleteComponent("indices", null, true)];
        ...
        let indices = ret.otherTokenValues[0];
        indices = indices.replace(/^"+/, '').replace(/"+$/, '');
        const length = ret.otherTokenValues?.length || 0;
        let autoField = true;
        ...
        for(let i=curTokenIdx - 1; i >=0; i--){
          const tok = ret.tokensLineCache[i];
          if(nearestToken === null && tok.value.trim() != ""){
            nearestToken = tok;
          }
          if(tok.type == "url.select_keyword" && tok.value.toUpperCase() != "SELECT"){
            autoField = false;
            break;
          }
        }
        if(autoField && (nearestToken.value != "," && nearestToken.value.toUpperCase() != "SELECT")){
          autoField = false;
        }
        switch(ret.otherTokenValues[length-1].toUpperCase()){
          case "SELECT":
        ...
        if(autoField && !ret.token!.value.includes('=')){
          const fields = getFields(indices);
          context.autoCompleteSet = addMetaToTermsList(
            fields,
            "field"
  ...
  function addUrlParamsAutoCompleteSetToContext(
    context: AutoCompleteContext,
    pos: Position
  ) {
    const ret = getCurrentMethodAndTokenPaths(editor, pos, parser);
  ...
  const evaluateCurrentTokenAfterAChange = _.debounce(
    function evaluateCurrentTokenAfterAChange(pos: Position) {
      ...
      try {
        currentToken = editor.getTokenAt(pos)!;
      } catch (e) {}
      ...
      if (!context) {
        ...
        const terms = _.map(
          context.autoCompleteSet!.filter(
            (term) => Boolean(term) && term.name != null
          ),
          function(term) {
            ...
            if (
              ...
              defaults.completer = {
                insertMatch() {
                  applyTerm(term);
                  editor.execCommand("exec_request");
                },
              };
            }
