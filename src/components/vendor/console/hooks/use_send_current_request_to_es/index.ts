...
import { getCommand } from "../../modules/mappings/mappings.js";
...
function buildRawCommonCommandRequest(cmd: any) {
  ...
  const strReqs = requests.map((req: any) => {
    ...
    if(["LOAD", "FROM", "SELECT"].includes((method || "").toUpperCase())){
      return body;
    }
    return `${method} ${path}\n${body}`;
  });
...
export const useSendCurrentRequestToES = (sendRequestToES) => {
