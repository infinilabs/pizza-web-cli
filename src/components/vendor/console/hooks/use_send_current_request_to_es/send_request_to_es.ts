      ...
      send(esMethod, esPath, esData, { clusterID: args.clusterID }).always(
        (dataOrjqXHR, textStatus: string, jqXhrORerrorThrown) => {
          ...
          if (isSuccess) {
            ...
            if (isMultiRequest) {
              if(req.url == "_sql" && req.rawRequest) {
                value = "# " + req.rawRequest + "\n" + value;
              }else{
            ...
            if (xhr.responseText) {
              ...
              if (resObj.error) {
                value = resObj.error.reason || resObj.error;
              ...
              value =
                "Request failed to get to the server (status code: " +
                xhr.status +
                ")";
              contentType = "text/plain";
            }
