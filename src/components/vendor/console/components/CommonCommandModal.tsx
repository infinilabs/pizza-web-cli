    ...
    (e) => {
      if (e.target.value.length == 0) {
        return message.warning(
          <FormattedMessage id="command.message.invalid.tag" />
        );
      }
    ...
    <div>
      ...
      {!inputVisible && (
        <Tag onClick={showInput} style={{ padding: "0 5px", fontSize: 14 }}>
          <PlusOutlined /> {<FormattedMessage id="command.btn.newtag" />}
        </Tag>
      )}
    </div>
  );
...
const CommonCommandModal = Form.create()((props: ICommonCommandModalProps) => {
  ...
  return (
    <Modal
      title={<FormattedMessage id="command.manage.save.title" />}
      visible={true}
      onCancel={props.onClose}
      onOk={handleConfirm}
      zIndex={1003}
      cancelText={<FormattedMessage id="form.button.cancel" />}
      okText={<FormattedMessage id="form.button.save" />}
    >
      <Form layout="vertical">
        <Form.Item label={<FormattedMessage id="command.table.field.name" />}>
          {form.getFieldDecorator("title", {
            rules: [{ required: true, message: "请输入标题" }],
          })(<Input />)}
        </Form.Item>
        <Form.Item label={<FormattedMessage id="command.table.field.tag" />}>
