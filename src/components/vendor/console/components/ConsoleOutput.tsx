...
} from "../modules/legacy_core_editor/create_readonly";
...
function ConsoleOutput({ clusterID }: props) {
  ...
  useEffect(() => {
    ...
    if (data) {
      if (!data.length) {
        editor.update(JSON.stringify(data, null, 2), "ace/mode/json");
        return;
      }
      ...
      if (mode == "ace/mode/json") {
        editor.update(
          JSON.stringify(JSON.parse(error.response.value), null, 2),
          mode
        );
        return;
      }
