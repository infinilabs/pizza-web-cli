...
const transformElasticsearchToOpensearch = (config: any) => {
  ...
  //rename
  [
    { from: "last_updated_time", to: "modified_date" },
    { from: "schema_version", to: "version" },
  ].forEach((item) => {
    if (config[item.to]) {
  ...
  policy["description"] = "tranform with INFINI Cloud";
...
const transformRollover = (rolloverCfg: any, reverse: boolean) => {
  [
    ...
    { from: "min_primary_shard_size", to: "max_primary_shard_size" },
    { from: "min_doc_count", to: "max_docs" },
    { from: "min_index_age", to: "max_age" },
  ].forEach((item) => {
    if (reverse) {
